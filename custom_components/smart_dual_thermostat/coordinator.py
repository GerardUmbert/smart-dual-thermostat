...
class ZoneCoordinator:
    ...
    async def async_turn_off(self) -> None:
        """Panic-button behavior: force both actuators off, reset forced mode."""
        self.active = False
    ...
    async def async_decide_and_act(self) -> None:
        ...
        if mode == DECIDED_MODE_HEAT and self.config.heat.entity_id:
    ...
    async def async_check_relax(self) -> None:
        ...
        if mode == DECIDED_MODE_COOL and self.config.cool.entity_id:
            current = self._actuator_current_temperature(self.config.cool)
            target = self._actuator_target_temperature(self.config.cool)
            if (
                current is not None
                ...
                await self._async_set_hvac_mode(self.config.cool, self.config.cool.hvac_mode)
                await self._async_set_temperature(self.config.cool, comfort.cool_relaxed)
                self._relaxed = True
                self._notify_listeners()
        elif mode == DECIDED_MODE_HEAT and self.config.heat.entity_id:
            indoor = self._indoor_temperature()
            target = self._actuator_target_temperature(self.config.heat)
            if (
                indoor is not None
                and target is not None
                ...
                await self._async_set_hvac_mode(self.config.heat, self.config.heat.hvac_mode)
                await self._async_set_temperature(self.config.heat, comfort.heat_relaxed)
                self._relaxed = True
                self._notify_listeners()
